import React from 'react'

const GenerateDID = () => {
  return (
    <div className='w-[1200px] mx-auto pb-[5rem]'>
        <p className='text-[24px] font-[600] mt-7'>Generating Your Digital Identity (DID)</p>
        <p className='text-[#464646] mt-2'>Your biodata is being securely processed to generate your DID on the blockchain.</p>
        <div className='flex flex-col items-center justify-center text-center mt-[4rem]'>
            <div className='w-[363px] h-[363px] bg-[#AAEA82] rounded-full'></div>
            <p className='w-[30%] text-center my-6'>Your Digital Identity is Ready!</p>
            <button className='bg-light-green w-[40%] mx-auto block text-white py-2 rounded-[8px] mt-3'>Proceed</button>
        </div>
    </div>
  )
}

export default GenerateDID